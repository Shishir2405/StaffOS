...
export function AutoGeofenceMonitor() {
  ...
  const fetchGeofences = async () => {
    try {
      ...
      const geofenceList = Array.isArray(data) ? data : (data.data || data.geofences || [])
  ...
  const findNearestZone = (latitude: number, longitude: number): Geofence | null => {
    for (const zone of geofences) {
      const distance = calculateDistance(latitude, longitude, zone.latitude, zone.longitude)
      console.log(`Distance to ${zone.name}: ${distance.toFixed(0)}m (radius: ${zone.radius}m)`)
  ...
  const autoCheckIn = async (zone: Geofence, latitude: number, longitude: number) => {
    if (!session?.user?.id) {
    ...
    try {
      ...
      const existingRecords = Array.isArray(existingData) ? existingData : (existingData.data || [])
  ...
  const autoCheckOut = async (latitude: number, longitude: number) => {
    ...
    try {
      ...
      if (records.length > 0 && !records[0].checkOutTime) {
        ...
        const updateResponse = await fetch(`/api/attendance?id=${attendanceId}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      (error) => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
          toast.error("Location permission denied", {
            description: "Please enable location access for auto check-in to work",
          })
        }
      },
      {
