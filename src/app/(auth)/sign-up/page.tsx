...
function getStrength(pw: string) {
  ...
  if (s <= 3) return { score: 3, label: "Good", color: "#0891B2" };
  return { score: 4, label: "Strong", color: "#16A34A" };
}

/* ─── Feature cards on the right panel ───────────────────── */
const featureCards = [
  {
    icon: RiGroupLine,
    title: "People Management",
    desc: "Unified profiles for every employee, from hire to retire.",
  },
  {
    icon: RiBarChartLine,
    title: "Real-time Analytics",
    desc: "HR dashboards that surface what matters instantly.",
  },
  {
    icon: RiBuildingLine,
    title: "Multi-location",
    desc: "Manage teams across offices, time zones, and countries.",
  },
];
...
function Field({
  ...
  return (
    <div>
      <label
        htmlFor={id}
        className="block mb-1.5 text-xs font-medium tracking-[0.10em] uppercase"
        style={{
        ...
        style={
          error
            ? {
                ...
                boxShadow: "0 0 0 3px oklch(0.578 0.232 13 / 0.10)",
              }
            : {}
        }
      >
        <span
          style={{
            color: error ? "var(--brand-rose)" : "var(--muted-foreground)",
            flexShrink: 0,
          }}
        >
          {icon}
...
export default function SignUpPage() {
  ...
  return (
    ...
    >
      {/* ── LEFT — Form ──────────────────────────────────── */}
      <section className="flex-1 flex items-center justify-center px-8 py-10 overflow-y-auto">
        <div className="w-full max-w-[400px]">
          ...
          >
            <h1
              className="text-[2.4rem] font-semibold mb-2 leading-[1.06]"
              style={{
                fontFamily: "var(--font-playfair)",
                color: "var(--foreground)",
              }}
            ...
            <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
              Set up your StaffOS workspace in seconds
            </p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="space-y-3.5">
                  ...
                  {form.password && (
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{ color: strength.color, minWidth: 38 }}
                      >
                        {strength.label}
                      </span>
                    </motion.div>
              ...
              <motion.div
                ...
                className="relative flex items-center gap-4 py-0.5"
              >
                <div
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(155deg, oklch(0.198 0.068 262 / 0.60) 0%, oklch(0.112 0.030 258 / 0.72) 100%)",
          }}
        />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-12 text-center">
          ...
          <motion.h2
            ...
            className="text-4xl font-bold text-white leading-[1.08] mb-6"
            style={{ fontFamily: "var(--font-playfair)" }}
          >
            Your people.
            <br />
            ...
            {featureCards.map((f, i) => (
              ...
              >
                <div
                  ...
                  style={{
                    background: "oklch(0.578 0.232 13 / 0.25)",
                    border: "1px solid oklch(0.578 0.232 13 / 0.35)",
                  }}
                >
                  <f.icon size={15} color="var(--brand-rose-muted)" />
                </div>
                <div>
          ...
          >
            <div className="flex -space-x-2">
              {["AM", "SK", "RV"].map((av, i) => (
                <div
                  key={av}
                  className="w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-[9px] font-medium text-white"
                  style={{
                    background:
                      i === 0
                        ? "var(--brand-rose)"
            ...
            <span className="text-xs text-white/70">
              Join <span className="text-white font-medium">2.4M+</span>{" "}
              employees already on StaffOS
            </span>
          </div>
        </motion.div>
      </motion.section>
