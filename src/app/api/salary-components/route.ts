...
export async function GET(request: NextRequest) {
  try {
    ...
    // List with filters
    let query = db.select().from(salaryComponents);
    const conditions = [];
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { employeeId, componentType, componentName, amount, isPercentage, isStatutory } = body;
    ...
    const newComponent = await db.insert(salaryComponents)
      .values({
        employeeId,
        componentType,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { componentType, componentName, amount, isPercentage, isStatutory } = body;

    // Validate componentType if provided
    if (componentType && !['basic', 'allowance', 'deduction'].includes(componentType)) {
      return NextResponse.json({
        error: 'Component type must be one of: basic, allowance, deduction',
        code: 'INVALID_COMPONENT_TYPE'
      }, { status: 400 });
    ...
    if (componentName !== undefined) updates.componentName = componentName.trim();
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete record
    const deleted = await db.delete(salaryComponents)
      .where(eq(salaryComponents.id, parseInt(id)))
      .returning();
