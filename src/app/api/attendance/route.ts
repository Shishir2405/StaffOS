...
function canAccessAttendanceData(
  userRole: string,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      ...
      // Check authorization
      if (
        !canAccessAttendanceData(userRole, userEmployeeId, record[0].employeeId)
      ) {
        return NextResponse.json(
          {
    ...
    if (userRole !== "admin" && userRole !== "hr") {
      ...
      conditions.push(eq(attendance.employeeId, userEmployeeId));
    } else if (employeeId) {
  ...
  try {
    ...
    if (userRole !== "admin" && userRole !== "hr") {
      if (!userEmployeeId || userEmployeeId !== employeeId) {
        return NextResponse.json(
          {
            error: "Forbidden - You can only create your own attendance",
            code: "FORBIDDEN",
          },
          { status: 403 },
        );
  ...
  try {
    ...
    if (
      !canAccessAttendanceData(
        userRole,
        userEmployeeId,
        existingRecord[0].employeeId,
      )
    ) {
      return NextResponse.json(
        {
          error: "Forbidden - You can only update your own attendance",
          code: "FORBIDDEN",
        },
        { status: 403 },
      );
    ...
    if (checkOutGeofenceId !== undefined)
      updates.checkOutGeofenceId = checkOutGeofenceId;
    if (checkOutLatitude !== undefined)
      updates.checkOutLatitude = checkOutLatitude;
    if (checkOutLongitude !== undefined)
      updates.checkOutLongitude = checkOutLongitude;
    ...
    if (
      workingHours !== null &&
      workingHours !== existingRecord[0].workingHours
    ) {
