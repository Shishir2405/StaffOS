...
function canAccessPayslipData(userRole: string, userEmployeeId: number | null, targetEmployeeId: number) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      ...
      // Check authorization
      if (!canAccessPayslipData(user.role, user.employeeId, record[0].employeeId)) {
        return NextResponse.json(
          { error: 'Forbidden - You can only access your own payslips', code: 'FORBIDDEN' },
          { status: 403 }
        );
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.role !== 'admin' && user.role !== 'hr') {
      return NextResponse.json(
        { error: 'Forbidden - Only admin and HR can create payslips', code: 'FORBIDDEN' },
        { status: 403 }
      );
    }
    ...
    const payrollRunExists = await db.select()
      .from(payrollRuns)
    ...
    for (const payslipData of payslipsData) {
      const result = await db.insert(payslips)
        .values({
          ...payslipData,
          payrollRunId,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (user.role !== 'admin' && user.role !== 'hr') {
      return NextResponse.json(
        { error: 'Forbidden - Only admin and HR can update payslips', code: 'FORBIDDEN' },
        { status: 403 }
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (user.role !== 'admin') {
      return NextResponse.json(
        { error: 'Forbidden - Only admin can delete payslips', code: 'FORBIDDEN' },
        { status: 403 }
      );
    }
