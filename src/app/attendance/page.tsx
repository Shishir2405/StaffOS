...
import { MapPin, Clock, CheckCircle2, XCircle, Navigation, Calendar, AlertCircle } from "lucide-react"
...
export default function AttendancePage() {
  ...
  const checkGeofenceAndAutoAttendance = async (location: { lat: number; lng: number }) => {
  ...
  const performAutoCheckIn = async (location: { lat: number; lng: number }, zone: GeofenceZone) => {
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const records = data.success && Array.isArray(data.data) ? data.data : (Array.isArray(data) ? data : [])
  ...
  const handleManualCheckIn = async () => {
    ...
    if (nearbyZones.length === 0) {
      toast.error("You are not within any assigned geofence zone.", {
        description: assignedGeofences.length === 0 
      ...
      <div className="flex flex-col gap-6 p-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            GPS-based attendance with automatic check-in/out • {assignedGeofences.length} assigned location{assignedGeofences.length !== 1 ? 's' : ''}
          </p>
        </div>
          ...
          <Card className="border-amber-500 bg-amber-50 dark:bg-amber-950/20">
            ...
            <CardContent>
              <p className="text-amber-800 dark:text-amber-200">
                You don't have any geofence locations assigned yet. Please contact your administrator to assign geofence locations for automatic attendance tracking.
              </p>
            </CardContent>
          </Card>
        )}
          ...
          <TabsContent value="tracking" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Auto-checking location every 3 seconds • 🟢 Auto attendance enabled
                </CardDescription>
              </CardHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    {isCheckedIn ? (
                      nearbyZones.length > 0 ? (
                        <div className="rounded-lg border bg-green-50 dark:bg-green-950/20 p-4">
                          <p className="font-medium text-green-900 dark:text-green-100 mb-2">
                            ✅ Checked in • Currently within {nearbyZones.length} zone(s):
                          </p>
                          <ul className="space-y-1">
                            {nearbyZones.map((zone) => (
                              <li key={zone.id} className="text-sm text-green-800 dark:text-green-200">
                                • {zone.name}
                              </li>
                            ))}
                          </ul>
                        ...
                        <div className="rounded-lg border bg-blue-50 dark:bg-blue-950/20 p-4">
                          ...
                          <p className="text-sm text-blue-800 dark:text-blue-200">
                            You'll be automatically checked out when system confirms you've left the area
                          </p>
                        </div>
                      )
                    ) : (
                      nearbyZones.length > 0 ? (
                        <div className="rounded-lg border bg-green-50 dark:bg-green-950/20 p-4">
                          <p className="font-medium text-green-900 dark:text-green-100 mb-2">
                            📍 You are within {nearbyZones.length} assigned zone(s):
                          </p>
                          <ul className="space-y-1">
                            {nearbyZones.map((zone) => (
                              <li key={zone.id} className="text-sm text-green-800 dark:text-green-200">
                ...
                {isCheckedIn ? (
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg border bg-green-50 dark:bg-green-950/20">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-green-700 dark:text-green-300">
                          You will be automatically checked out when you leave the geofence zone
                        </p>
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <Button
                      ...
                      disabled={isLoading || nearbyZones.length === 0 || !currentLocation}
                      className="w-full"
                      size="lg"
                    >
                    ...
                    {nearbyZones.length === 0 && assignedGeofences.length > 0 && (
                      <p className="text-sm text-center text-muted-foreground">
                        You must be within an assigned geofence zone to check in
                      </p>
                    )}
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 rounded-lg border bg-muted/50">
                      <div>
                        ...
                        <p className="text-3xl font-bold">{dailySummary.totalWorkingHours.toFixed(2)}h</p>
                      </div>
                      <Badge variant={dailySummary.status === "Present" ? "default" : "destructive"}>
                        {dailySummary.status}
                      </Badge>
                    </div>
                        ...
                        <TableBody>
                          {dailySummary.checkInOut.length === 0 ? (
                            ...
                            dailySummary.checkInOut.map((pair, index) => (
                              <TableRow key={index}>
                                <TableCell>
                                  {pair.checkInTime ? formatTime(pair.checkInTime) : "-"}
                                </TableCell>
                                <TableCell>
                                  {pair.checkOutTime ? formatTime(pair.checkOutTime) : "Active"}
                                </TableCell>
                                <TableCell>
                                  {pair.duration > 0 ? `${pair.duration.toFixed(2)}h` : "-"}
                                </TableCell>
                              </TableRow>
                            ))
                          )}
